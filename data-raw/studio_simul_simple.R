...
load(file.path(DIR, "sim12_simple.rda"))
...
likfun <- function(x, i, j, y = par.stime[, i]) {
  ...
  y[j] <- x
  if (any(is.nan(y))) stop("rilevati NAN!")
...
for (i in 1:ncol(par.stime)) {
  ...
  for (j in 1:nrow(par.stime)) {
    ...
    # ottimizzazione univariata (scala trasformata) fissando gli altri parametri
    #if (j >= 14) browser()
...
save(lik.vals,lik.vals2, par.stime, par.stime2, par.sd, par.sd2, par.sd22, par.stime22,
     errori.stima.mv, file = file.path(DIR, "sim12_simple.rda"))
...
set.seed(32)
idx <- sample(which(errori.stima.mv > .05), 12)
...
save(trv, file = file.path(DIR, "sim12_simple_trv.rda"))

#| eval: true
load(file.path(DIR, "sim12_simple_trv.rda"))
apply(trv, c(1, 3), \(x) mean(is.finite(x)))

#| fig-cap: "Intervalli di confidenza al 95% basati sulla verosimiglianza per i parametri."
#| fig-asp: 2
par(mfrow = c(5, 3))
for (j in seq_len(nrow(par.stime))) {
